...
func Debugtool(confs []*TailTaskConfig) (ret string) {
	if len(confs) == 0 {
		return "[]"
	}
	for _, conf := range confs {
		ret += fmt.Sprintf("%s to %s, ", conf.Path, conf.Topic)
	}
...
func addTask(entry *TailTaskConfig) (err error) {
	...
	golog.Debug("[tailMgr] Success to add task: ", taskinfo)
	...
	for {
		...
		deleteLoop:
			for _, oldEntry := range oldEntrys {
				...
				golog.Debug("[tailMgr] Cancel old task: ", oldTaskinfo)
				instance.taskMap[oldTaskinfo].cancel()
			}
			instance.configs = newEntrys
		default:
