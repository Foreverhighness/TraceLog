...
func GetConfig(key string) (configs []*taillog.TailTaskConfig, err error) {
	...
	for _, kvs := range responses.Kvs {
		golog.Info("[etcd] Get key: %v, value: %v", kvs.Key, kvs.Value)
...
func debugtool(confs []*taillog.TailTaskConfig) (ret string) {
	if len(confs) == 0 {
		return "[]"
	}
	for _, conf := range confs {
		ret += fmt.Sprintf("%s_%s, ", conf.Path, conf.Topic)
	}
...
func BindingConfigChannel(key string, configChan chan<- []*taillog.TailTaskConfig) {
	...
	for response := range watchChan {
		for _, event := range response.Events {
			golog.Debugf("[etcd] Get response from watchChannel, type:%v, key:%v, value:%v", event.Type, event.Kv.Key, event.Kv.Value)
